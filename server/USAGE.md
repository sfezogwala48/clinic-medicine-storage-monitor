...
Auth is credential-based (JWT): `POST /api/auth/login` with `{"identifier": "<contact-or-name>", "password": "..."}` returns `{token, user}`. Pass an optional `"role"` when the client pre-selected a user type — the account must belong to it. Pass it as `Authorization: Bearer <token>` on every `/api/*` request — unauthenticated calls get `401`. Tokens expire after `JWT_EXPIRES_IN` (default `12h`); signing secret is `JWT_SECRET`. Both the secret and the default admin credentials live in `server/.env` (see `server/.env.example`) — the server refuses to boot without them, and there are no hardcoded fallbacks. First boot seeds `admin@clinic.co.za`, `+27721111111` (supervisor), `+27730000000` (staff) from `SEED_<ROLE>_PASSWORD`. Prefer `pnpm bootstrap` (dev) or `pnpm bootstrap:prod` (source build), which create `.env` from the example and fill the secrets. Public without a token: `/health*`, `/openapi.json`, `/reference`, `/telemetry/*` (devices), and `POST /api/auth/login`.
